...
async function searchTMDB(title, type, key) {
  try {
    ...
    if (key) {
      ...
      // 如果平台没有注入 Widget.tmdb，这里可能报错，可酌情增加检查（不在本次需求内）
    ...
    var yearGenre = year ? year + "·" + genres : genres;   // 年份·类型
...
async function loadAIList(params) {
  ...
  try {
    ...
    if (names.length === 0) {
      return [{ 
        id: "err", type: "tmdb", title: "AI 未能生成列表", 
        subTitle: "错误", description: "请检查 API Key 或模型设置" 
      }];
    }
    ...
    return [{ 
      id: "err", type: "tmdb", title: "请求出错", 
      subTitle: "异常", description: e.message 
    }];
  }
}
